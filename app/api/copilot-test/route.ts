  ...
  const diagnostics = {
    ...
    environment: {
      neo4j: {
        ...
        password: process.env.NEO4J_PASSWORD ? '***SET***' : 'Not set (will use default: REDACTED_PASSWORD)',
...
function generateRecommendations(diagnostics: any): string[] {
  ...
  if (neo4jTest.status !== 'connected') {
    ...
    if (neo4jTest.error?.includes('authentication')) {
      recommendations.push('Reset Neo4j password: `cypher-shell` then `ALTER USER neo4j SET PASSWORD \'REDACTED_PASSWORD\'`');
    }
