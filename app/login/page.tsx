...
export default function LoginPage() {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          const mfaData: LoginResponse =
            await mfaResponse.json()
          if (mfaData.secretCode) {
            ...
            setValidMfaCodes(
              Array.isArray(mfaData.validCodes)
                ? mfaData.validCodes
                : mfaData.validCodes || []
            )
  ...
  const handleMFASetup = async () => {
    ...
    try {
      if (
        !setupMfaCode ||
        !/^\d{6}$/.test(setupMfaCode)
      ) {
        const serverCode =
          await synchronizeTimeAndGetCode()
        if (serverCode) {
          setSetupMfaCode(serverCode)
          setSuccessMessage(
            "Using server-generated verification code..."
          )
        } else {
          setError(
            "Failed to get verification code from server. Please try again."
          )
          setIsLoading(false)
          return
        }
      ...
      if (!response.ok) {
        throw new Error(
          data.detail ||
            `Failed to verify MFA setup (${response.status})`
        )
      }
      ...
      if (data.access_token) {
        ...
        setSuccessMessage(
          "MFA setup successful. Please log in again."
        )
      }
    } catch (error: any) {
      setError(
        error.message || "Failed to set up MFA"
      )
    } finally {
      setIsLoading(false)
    }
  ...
  const handleMFASubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.detail || `Failed to verify MFA (${response.status})`
        )
      }
