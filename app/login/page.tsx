...
export default function LoginPage() {
  ...
  const handleMFASetup = async () => {
    ...
    try {
      ...
      // Wait for the next 30-second TOTP window if this is a retry
      // This helps with timing issues between client and server
      if (mfaSetupAttempts > 0) {
        const now = Math.floor(Date.now() / 1000)
        const secondsToNextWindow = 30 - (now % 30)

        if (secondsToNextWindow < 5) {
          logApiCall("MFA Setup Timing", `Waiting ${secondsToNextWindow + 2} seconds for next TOTP window`)

          // Set a timeout to wait for the next window
          const timeoutId = window.setTimeout(
            () => {
              logApiCall("MFA Setup Timing", "TOTP window changed, proceeding with verification")
              submitMfaVerification(accessToken)
            },
            (secondsToNextWindow + 2) * 1000,
          )
      ...
      await submitMfaVerification(accessToken)
    } catch (error) {
  ...
  // Extracted the MFA verification logic to a separate function
  const submitMfaVerification = async (accessToken: string | null) => {
    let endpoint
    let requestBody

    // Determine which API endpoint and request structure to use based on available credentials
    if (accessToken) {
      // Use the access token flow - this is the preferred method
      ...
      // Use the session-based flow as fallback
    ...
    // Add a timestamp to help with debugging time-related issues
    const clientTime = new Date()
    logApiCall(
      "MFA Setup Timing",
      `Client time: ${clientTime.toISOString()}, TOTP window: ${Math.floor(clientTime.getTime() / 30000)}`,
    )
    ...
    const response = await fetchWithRetry(
      ...
      2,
      20000,
    ) // Increased timeout and retries for MFA verification
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 && responseData.detail) {
        ...
        logApiCall(
          ...
          responseData.detail || `Failed to verify MFA setup (${response.status})`,
        )
  ...
  // Helper to check time synchronization
  const checkTimeSync = () => {
    try {
      const now = new Date()
      const secondsInWindow = now.getSeconds() % 30
      const secondsToNextWindow = 30 - secondsInWindow

      logApiCall(
        "Time Sync Check",
        `Client time: ${now.toISOString()}\n` +
          `Seconds in current window: ${secondsInWindow}\n` +
          `Seconds to next window: ${secondsToNextWindow}`,
      )

      setError(
        `Time check: ${secondsInWindow} seconds into current window, ${secondsToNextWindow} seconds until next code.`,
      )
      ...
      logApiCall("Time Sync Check Error", error.message || "Unknown error")
    }
  }
        ...
        >
          <CardContent className="space-y-6">
            <RadioGroup value={userType} onValueChange={(value: UserType) => setUserType(value)} className="grid gap-4">
              <div className="relative flex items-center space-x-4 rounded-lg border border-border bg-secondary p-4 hover:border-primary">
            ...
            {error && (
              <Alert className="bg-secondary border-border" variant="destructive">
        ...
        <DialogContent className="bg-card border-border">
          ...
          <div className="grid gap-4 py-4">
            ...
            {qrCodeUrl && (
              <div className="flex flex-col items-center justify-center space-y-2">
                ...
                <div className="bg-white p-2 rounded-md w-48 h-48 mx-auto">
                  <img
            ...
            {mfaSecretCode && (
              <div className="space-y-2 text-center">
                ...
                <div className="p-3 bg-secondary border border-border rounded-md font-mono text-center break-all">
                  {mfaSecretCode}
                </div>
                <p className="text-xs text-muted-foreground">Enter this code manually if you cannot scan the QR code</p>
              </div>
            )}
          ...
          <DialogFooter className="flex flex-col gap-3 sm:flex-row">
            <Button
              ...
              disabled={isLoading}
              className="sm:w-auto w-full bg-secondary border-border hover:bg-secondary/80 hover:border-primary"
            >
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  className="flex-1 bg-secondary border-border hover:bg-secondary/80 hover:border-primary"
                >
                  Back
                </Button>
                <Button
