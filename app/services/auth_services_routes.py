...
class LoginResponse(BaseModel):
    token: str = None  # Make token optional
    role: str = None   # Make role optional
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if auth_response.get("mfa_required"):
        ...
        return {
            "token": None,
            "role": None,
            "email": form_data.username,  # Or None if email isn't available during MFA
            "mfa_required": True,
    ...
    return {
        ...
        "role": "employee",  # Or get the role from auth_response if available
...
async def verify_mfa_endpoint(request: Request, mfa_request: MFARequest):
    try:
        ...
        return {"token": auth_result["IdToken"]}  # Return only the token
    except Exception as e:
