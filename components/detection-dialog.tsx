...
export function DetectionDialog({
    ...
    const loadTeamMembers = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            "✅ Loaded team members for assignment dialog:",
            users.length,
          );
        } else {
  ...
  const removeAssignee = (
    ...
    setSelectedAssignees((current) =>
      current.filter((assignee) => assignee !== assigneeToRemove),
    );
  };
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <span className="col-span-3">
              <Badge
                className={cn(
                  "w-16",
                  getSeverityBadgeClass(detection.severity),
                )}
              >
                {detection.severity}
              </Badge>
