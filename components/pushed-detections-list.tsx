...
interface PushedDetectionsListProps {
  searchQuery: string
  pushedDetections: Detection[]
}

export function PushedDetectionsList({ searchQuery, pushedDetections }: PushedDetectionsListProps) {
  ...
  const filteredDetections = useMemo(() => {
    return pushedDetections.filter((detection) => {
      const matchesSearch =
        ...
        detection.pushedBy.toLowerCase().includes(searchQuery.toLowerCase())
      ...
      const matchesPushedBy = filters.pushedBy === "All" || detection.pushedBy === filters.pushedBy

      const matchesTimeRange = filters.timeRange === "All"

      return (
        matchesSearch && matchesSeverity && matchesStatus && matchesAssignedTo && matchesPushedBy && matchesTimeRange
      )
    })
  }, [searchQuery, pushedDetections, filters])
  ...
  return (
    <>
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-[#1f1f1f] border-[#1f1f1f]">
              ...
              <TableHead className="w-[200px] text-white">
                <div className="flex items-center space-x-2">
                  <span>Pushed From</span>
                  {renderFilterPopover("pushedBy", ["All", ...new Set(pushedDetections.map((d) => d.pushedBy))])}
                </div>
              </TableHead>
          ...
          <TableBody>
            {filteredDetections.map((detection) => (
              ...
              >
                <TableCell className="text-white">{detection.uniqueId}</TableCell>
                <TableCell>
                  <Badge className={cn("w-20", getSeverityBadgeClass(detection.severity))}>{detection.severity}</Badge>
                </TableCell>
                <TableCell className="font-medium text-white">{detection.name}</TableCell>
                <TableCell className="text-white">{detection.status}</TableCell>
                ...
                <TableCell className="text-white">{detection.pushedBy}</TableCell>
                <TableCell className="text-right text-white">{detection.timestamp}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      <DetectionDialog
        ...
        onAssign={() => {}} // This is a no-op since we don't want to allow assignment in pushed requests
      />
    </>
  )
}
