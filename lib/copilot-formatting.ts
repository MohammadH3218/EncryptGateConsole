...
export function parseEmailReferences(text: string): EmailReference[] {
  ...
  const pattern1 =
    /(Email|Message ID|Email ID|MessageID):\s*<?([^>\s]+@[^>\s]+)>?/gi;
  ...
  while ((match = pattern3.exec(text)) !== null) {
    ...
    const alreadyExists = references.some(
      (ref) =>
        ref.startIndex <= match!.index &&
        ref.endIndex >= match!.index + match![0].length,
    );
...
export function formatMarkdown(text: string): string {
  ...
  formatted = formatted.replace(
    /`(.+?)`/g,
    '<code class="bg-neutral-800 px-1.5 py-0.5 rounded text-sm text-blue-400">$1</code>',
  );
...
export function formatSecurityContent(text: string): string {
  ...
  formatted = formatted.replace(
    /\b(Critical|High|Medium|Low)\s+(Risk|Priority|Severity)\b/gi,
    (match) => {
