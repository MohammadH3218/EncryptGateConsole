...
# Enable Flask-CORS on all routes under /api/
CORS(
    app,
...
@app.route("/api/auth/test-mfa-code", methods=["POST"])
def direct_test_mfa_code():
    ...
    try:
        ...
        # Parse client_time and adjusted_time if provided
        ...
        # If session is provided, try to get a secret code from it (for software token MFA)
        ...
        # If no secret at all, return time sync info
        ...
        try:
            ...
            # Generate multiple valid codes for ±5 windows
            valid_codes = []
            for i in range(-5, 6):
            ...
            # If no code provided, return current code + sync info
            if not code:
                return jsonify({
                    "valid": True,
            ...
            # Verify provided code within extended window
...
def direct_authenticate():
    ...
    try:
        ...
        try:
            ...
            hash_obj = hmac.new(secret_bytes, message.encode("utf-8"), hashlib.sha256)
...
def direct_respond_to_challenge():
    ...
    try:
        ...
        # Build ChallengeResponses payload
        ...
        logger.error(traceback.format_exc())
        return jsonify({
            "detail": f"Challenge response failed: {str(e)}"
        }), 500
...
@app.route("/api/auth/confirm-mfa-setup", methods=["POST"])
def direct_confirm_mfa_setup():
    ...
    try:
        ...
        # Step 1: Associate software token again to get new session / secret
        try:
        ...
        try:
            ...
            # Check adjusted client time if provided
            client_code = None
            if adjusted_time_str:
                try:
                    adjusted_dt = parse_iso_datetime(adjusted_time_str)
            ...
            # If code doesn’t match any valid window, replace with server code
            if not (is_valid_server or is_valid_client):
                ...
                code = current_code
                logger.info(f"Replaced user code {code} with server code {code}")

        except Exception as totp_error:
            logger.error(f"TOTP error during verification: {totp_error}")
            return jsonify({"detail": "Invalid or expired verification code."}), 400

        # Step 2: Verify software token with Cognito
        ...
        try:
            ...
            if not successful_code:
                ...
                return jsonify({
                    ...
                    "validCodes": valid_codes[4:7],  # 3 codes closest to current time
                    "timeInfo": {
        ...
        # Step 3: If password provided, attempt final USER_PASSWORD_AUTH to issue tokens
        if password:
            try:
                CLIENT_ID = os.getenv("COGNITO_CLIENT_ID")
                CLIENT_SECRET = os.getenv("COGNITO_CLIENT_SECRET")
...
def direct_verify_mfa():
    ...
    try:
        ...
        # We skip generating a new secret here, since user should already have MFA configured.
        CLIENT_ID = os.getenv("COGNITO_CLIENT_ID")
        CLIENT_SECRET = os.getenv("COGNITO_CLIENT_SECRET")
        if not CLIENT_ID or not CLIENT_SECRET:
            logger.error("AWS Cognito configuration missing")
        ...
        try:
            ...
            logger.error(f"Secret hash error: {hash_error}")
            return jsonify({"detail": "Verification failed: Unable to generate credentials"}), 500
        ...
        try:
            ...
            # If Cognito returns CodeMismatch, just tell user to retry
            return jsonify({
            ...
            logger.error(f"Cognito API call failed: {e}")
            return jsonify({"detail": f"MFA verification failed: {str(e)}"}), 500
